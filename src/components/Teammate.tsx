import '../styles/teammate.scss';

interface TeammateProps {
  name: string;
  github: string;
  linkedin: string;
  picture: string;
}

const Teammate = ({ name, github, linkedin, picture }: TeammateProps) => {
  // if clicked takes us to user linkedin page
  const handleClickLinkedIn = (url: string) => window.open(url, '_blank');

  // if clicked takes us teammates github
  const handleClickGithub = (url: string) => window.open(url, '_blank');

  // teammates information is all received from props of teams
  return (
    <div className='team-information'>
      <img
        alt={`${name}'s profile picture`}
        className='profile-pic'
        src={picture}
      />
      <div className='NameSocial'>
        <p className='name'>{name}</p>
        <div>
          <img
            alt={`URL to ${name}'s LinkedIn profile`}
            className='linkedin'
            onClick={() => handleClickLinkedIn(linkedin)}
            src='data:image/png;base64,'
          />
          <img
            alt={`URL to ${name}'s GitHub profile`}
            className='github'
            onClick={() => handleClickGithub(github)}
            src='https://i.pinimg.com/564x/b5/1b/78/b51b78ecc9e5711274931774e433b5e6.jpg'
          />
        </div>
      </div>
    </div>
  );
};

export default Teammate;
